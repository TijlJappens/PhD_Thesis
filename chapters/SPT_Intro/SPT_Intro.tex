...
\begin{itemize}
	\item If $h/J\in]-\infty,-[\:\cup\:]1,+\infty[$, $H_{J,h}$ has a unique gapped groundstate $\ket{\psi_{J,h}}$ such that $U\ket{\psi_{J,h}}=\ket{\psi_{J,h}}$.
	\item If $h/J\in]-1,1[$, $H_{J,h}$ has a two fold degenerate gapped groundstate given by $\ket{\psi_{J,h}^1}$ and $\ket{\psi_{J,h}^2}$\footnote{To put it in the light of what proceeds, if we ask that $\ket{\psi_{J,h}^1}$ and $\ket{\psi_{J,h}^2}$ are short ranged entangled states (see section \ref{sec:GappedPhasesOfMatterIntro}) then these two states are unique (superpositions of them are not short range entangled).} such that $U\ket{\psi_{J,h}^1}=\ket{\psi_{J,h}^2}$.
\end{itemize}
We call the region of the parameter space where the Hamiltonian has a unique gapped groundstate the symmetry preserved phase of matter while we call the region where the Hamiltonian has a two fold degenerate groundstate the symmetry broken phase of matter.\\\\
One of the key features of such a Landau phase of matter is that one can measure the phase transition with local order parameters. In this model, we have a local order parameter of the form $\sigma_{z,i}$ (for some site $i$). To show this, we will calculate the correlation function of the order parameter in the ground state (density matrix). More specifically, let
\begin{equation}
	\omega_{J,h}(\cdot)\defeq \left\{\begin{matrix}
		\bra{\psi_{J,h}}\cdot \ket{\psi_{J,h}}&\text{if }h/J\in]-\infty,-[\:\cup\:]1,+\infty[\\
		\frac{1}{2}\left(\bra{\psi_{J,h}^1}\cdot \ket{\psi_{J,h}^1}+\bra{\psi_{J,h}^2}\cdot \ket{\psi_{J,h}^2}\right)&\text{if }h/J\in]-1,1[.
	\end{matrix}\right.
\end{equation}
be the ground state (for $h/J\notin \{-1,+1\}$). If we calculate the correlation function of the order parameter with respect to this groundstate:
\begin{equation}
	Cor_{J,h}(\sigma_{z,i},\sigma_{z,j})\defeq \omega_{J,h}(\sigma_{z,i}\sigma_{z,j})-\omega_{J,h}(\sigma_{z,i})\omega_{J,h}(\sigma_{z,j})
\end{equation}
we obtain that
\begin{equation}
	Cor_{J,h}(\sigma_{z,i},\sigma_{z,j})=\left\{\begin{matrix}
		1&\text{if }h/J\in]-\infty,-[\:\cup\:]1,+\infty[\\
		0&\text{if }h/J\in]-1,1[.
	\end{matrix}\right.
\end{equation}
This shows that the local order parameter can measure the difference between the symmetry preserving phase (where the correlation function is 0) and the symmetry broken phase (where the correlation is $1$).

\section{Uniquely gapped phases of matter}\label{sec:GappedPhasesOfMatterIntro}
...
\end{conclusion}
\subsection{No \texorpdfstring{$G$}{G}-invariant finite depth quantum circuits}
\begin{figure}
	\centering
	\scalebox{0.65}{
...
\subsection{Example 2: \texorpdfstring{$H^2(G,\TT)$}{H2}}\label{sec:example-2-h2gtt}
