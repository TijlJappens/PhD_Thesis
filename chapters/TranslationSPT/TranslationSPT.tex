% !TeX root = ../../thesis.tex
\chapter{SPT indices emerging from translation invariance in two-dimensional quantum spin systems}\label{ch:TranslationSPT}
...
A final remark:
\begin{remark}
	As noticed in \cite{Chen_2013}. The groups in which these indices take values can be concisely written out. We use the connection with cohomology taking values in $\ZZ$ of the form $H^d(G,U(1))\cong H^{d+1}(G,\ZZ)$ (for $d>0$). We will also use the fact that
	\begin{align}
		\nonumber
		H^0(\ZZ,\ZZ)&\cong \ZZ& H^1(\ZZ,\ZZ)&\cong \ZZ& H^2(\ZZ,\TT)&\cong 0&H^3(\ZZ,\ZZ)&\cong 0&H^4(\ZZ,\ZZ)&\cong 0
	\end{align}
	and inserting this into the K\"unneth formula\footnote{The $\textrm{Tor}^\ZZ_1$ part of the K\"unneth formula is trivial because $\ZZ$ is free.} for $\ZZ\times G$ gives
	\begin{align}
		\nonumber
		H^3(\ZZ \times G,\ZZ)&\cong \bigoplus_{i=0}^3 H^i(\ZZ,\ZZ)\otimes H^{3-i}(G,\ZZ)\cong H^3(G,\ZZ)\oplus H^2(G,\ZZ)\\
		\nonumber
		H^4(\ZZ \times G,\ZZ)&\cong \bigoplus_{i=0}^4 H^i(\ZZ,\ZZ)\otimes H^{4-i}(G,\ZZ)\cong H^4(G,\ZZ)\oplus H^3(G,\ZZ).
	\end{align}
	This already allows us to write out the case with one translation index more concisely. This result can in turn be used to work out the K\"unneth formula for $\ZZ^2\times G$
	\begin{align}
		\nonumber
		H^4(\ZZ^2\times G,\ZZ)&\cong \bigoplus_{i=0}^4 H^i(\ZZ,\ZZ)\otimes H^{4-i}(\ZZ\times G,\ZZ)\\
		\nonumber
		&\cong H^4(G,\ZZ)\oplus H^3(G,\ZZ)\oplus H^3(G,\ZZ)\oplus H^2(G,\ZZ).
	\end{align}
	This allows us to write out the case with two translation indices more concisely.
\end{remark}
...
\begin{definition}\label{def:S_i_WithEquivelenceRelation}
...
\begin{align}
	...
	\nonumber
	S_3&=\{(\omega,U)\in\PP(\AA)\times\hom(G,U(\CC^d))|\omega\text{ is SRE},\:\omega\circ i_{U(g)}=\omega\:\forall g\in G,\:\omega\circ\tau=\omega\circ\nu=\omega\}
\end{align}
...
\begin{figure}
	...
	\resizebox{0.32\textwidth}{!}{%
		\begin{tikzpicture}
			...
			\draw[draw=black,->,line width=0.3mm] (2.5*\s,0) arc (0:45:1.5*\s);
...
\end{figure}
Similarly what was done in \cite{Ogata2d}, we will define an index on the space of states that can be disentangled by a Q-automorphism. This class of automorphisms is defined as:
...
\end{proof}
\subsection{Comparison with \texorpdfstring{\cite{Ogata2d}}{}}\label{sec:SomeRemarks}
...
\begin{figure}
	...
	\resizebox{0.2\textwidth}{!}{%
		\begin{tikzpicture}
			...
			\draw[draw=black,->,line width=0.3mm] (1.5*\s,1*\s) arc (0:45:1.5*\s);
	...
	\resizebox{0.2\textwidth}{!}{%
		\begin{tikzpicture}
			...
			\draw[draw=black,->,line width=0.3mm] (1.5*\s,0) arc (0:45:1.5*\s);
	...
	\resizebox{0.2\textwidth}{!}{%
		\begin{tikzpicture}
			...
			\draw[draw=black,->,line width=0.3mm] (2.5*\s,1*\s) arc (0:45:1.5*\s);
...
\end{proof}
Following Ogata \cite{Ogata2d}, we now define objects using the following lemma:
...
\end{proof}
We the next goal is to prove that this function $C$ is a $2$-cochain. We will do this by proving that the $3$-cochain constructed in \cite{Ogata2d} is invariant under some substitution. More precisely, we will use:
...
\begin{proof}
	...
	\begin{align}
		...
		\nonumber
		&=v^\dagger \delta_g^R v K_{g,1}^R (\delta_g^R)^\dagger \delta_g W_{g,1}v^\dagger \delta_h^R v K_{h,1}^R (\delta_h^R)^\dagger W_{g,1}^\dagger \delta_g^\dagger \delta_g^R W_{g,1}\delta_h^R W_{g,1}^\dagger u_{R,1}(g,h)\\
		\nonumber
		&\qquad \stkout{(\delta_{gh}^R)^\dagger \delta_{gh}^R} (K_{gh,1}^R)^\dagger  v^\dagger \stkout{(\delta_{gh}^R)^\dagger v  v^\dagger \delta^R_{gh}} u_{R,1}(g,h)^\dagger W_{g,1}(\delta^R_h)^\dagger W_{g,1}^\dagger(\delta_g^R)^\dagger v.
	\end{align}
	We now insert $(K_{h,1}^R)^\dagger W_{g,1}^\dagger (K_{g,1}^R)^\dagger K_{g,1}^R W_{g,1}K_{h,1}^R=\id:$
	\begin{align}
		\nonumber
		C_2(g,h)&=v^\dagger \delta_g^R v K_{g,1}^R (\delta_g^R)^\dagger \delta_g W_{g,1}v^\dagger \delta_h^R v K_{h,1}^R (\delta_h^R)^\dagger W_{g,1}^\dagger \delta_g^\dagger\delta_g^R W_{g,1}\delta_h^R \\
		&\qquad \left((K_{h,1}^R)^\dagger W_{g,1}^\dagger (K_{g,1}^R)^\dagger K_{g,1}^R W_{g,1}K_{h,1}^R\right)\\
		\nonumber
	...
	\begin{align}
		...
		\nonumber
		&=C_1(g,h)v^\dagger \delta_g^R v K_{g,1}^\stkout{R} W_{g,1}v^\dagger \delta_h^R v \stkout{K_{h,1}^R (\delta_h^R)^\dagger W_{g,1}^\dagger W_{g,1}\delta_h^R (K_{h,1}^R)^\dagger}\\
		\nonumber
		&\qquad W_{g,1}^\dagger (K_{g,1}^\stkout{R})^\dagger v^\dagger W_{g,1}(\delta^R_h)^\dagger W_{g,1}^\dagger(\delta_g^R)^\dagger v\\
		\nonumber
	...
	\begin{align}
		...
		\nonumber
		&=v^\dagger \left(w^\dagger\delta^R_g w b_{g,1}^R (\delta^R_g)^\dagger\right) v h_g \left(v^\dagger \delta_g^R v K_{g,1}^R (\delta_g^R)^\dagger\right) \left(w^\dagger\delta^R_g w b_{g,1}^R (\delta^R_g)^\dagger\right)^\dagger \\
		&\qquad w^\dagger (v^\dagger \delta_g^R v K_{g,1}^R (\delta_g^R)^\dagger)^\dagger w\\
		\nonumber
...
\begin{proof}
	...
	\begin{align}
		\nonumber
		&=\pi_0\circ\alpha_0\circ\Theta\circ\Ad{a}\circ\xi_1\circ\Phi(A_2^\dagger)=\pi_0\circ\alpha_0\circ\Theta\circ\Ad{a}\circ\Phi\circ\xi_1(A_2^\dagger)\\
...
\begin{proof}
	...
	\begin{align}
		...
		\nonumber
		&=\pi_0\circ\alpha_0\circ\Theta\circ\gamma^{H}_{0;1}\circ\gamma^{\Hsplit}_{1;0}\circ\tilde{\eta}_g\circ\beta_g^U(A)\tilde{a}\tilde{W}_g\stkout{y\tilde{W}_g^\dagger\tilde{a}^\dagger}\stkout{\tilde{a}\tilde{W}_g\tilde{a}^\dagger\tilde{a}y^\dagger}\tilde{a}^\dagger\\
		\nonumber
		&\qquad \pi_0\circ\alpha_0\circ\Theta\circ\gamma^{H}_{0;1}\circ\gamma^{\Hsplit}_{1;0}(A)^\dagger\tilde{a}\tilde{W}_g^\dagger\tilde{a}^\dagger.
	\end{align}
...
\begin{proof}
	...
	\begin{align}
		...
		&=\pi_0\circ\alpha_0\circ\Theta\circ\gamma^H_{0;1}\circ\gamma^{\Hsplit}_{1;0}\\
		\nonumber
		...
		&\qquad\circ(\beta_g^{\nu^{-1}(RU)})^{-1}\circ(\eta_g^R)^{-1}\circ\gamma^{\Hsplit}_{0;1}\circ\gamma^{H}_{1;0}.
	\end{align}
	Inserting twice the identity in this gives
	\begin{align}
	...
	\begin{align}
		...
		\nonumber
		&=\pi_0\circ\alpha_0\circ\Theta\circ\gamma^H_{0;1}\circ\nu^{-1}\circ\tilde\eta_g^R\circ\beta_g^{\nu(RU)}\circ\nu\circ\gamma^{\Hsplit}_{1;0}\\
		\nonumber
		&\qquad \circ(\beta_g^{RU})^{-1}\circ(\eta_g^R)^{-1}\circ\gamma^{\Hsplit}_{0;1}\circ\Ad{(A_{2,g}^R)^\dagger}\circ\gamma^{H}_{1;0}\\
		\nonumber
...
\begin{proof}
	...
	\begin{align}
		...
		\nonumber
		&\otimes \left(\pi_{0,3}\otimes\pi_{0,4}\circ\gamma^{\Phi_3+\Phi_4}_{0;1}\circ\eta_g^L\circ\tilde{\eta}_g^D\circ(\beta_g^{LD})^{-1}\circ\gamma^{\Phi_3+\Phi_4}_{1;0}\right).
	\end{align}}%
...
\begin{proof}
	...
	\begin{align}
		...
		&=\Ad{(b_g^R)^\dagger}\circ\pi_0\circ\gamma^\Phi_{0;1}\circ\nu^{-1}\circ\tilde{\eta}_g^U\circ\eta_g^R\circ\beta_g^{RU}\\
		\nonumber
...
\begin{align}
	C(g,h)&=(K_{g,1}^R\otimes K_{g,2}^R)\Ad{W_{g,1}\otimes W_{g,2}}\left((K_{h,1}^R\otimes K_{h,2}^R)\right.\\
	\nonumber
	&\qquad\left.\Ad{(W_{h,1}\otimes W_{h,2})(W_{gh,1}\otimes W_{gh,2})^\dagger}\left((K_{gh,1}^R\otimes K_{gh,2}^R)^\dagger\right)\right)\\
...
\end{definition}
As a remark, notice that this definition is not identical to the de definition given in \cite{Ogata2d} due to the translation operators. Before we prove the theorem, we will first list some required lemmas along with their proof.
