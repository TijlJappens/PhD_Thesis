...
\begin{figure}
	...
	\end{subfigure}
	\caption{Four examples of topological order.}
...
\\\\
The materials exhibiting these Hall plateaus possessed intriguing new properties: they were classified by an integer, now known as the topological invariant or Chern number. Unlike conventional order parameters, this integer remained robust under local perturbations. Strikingly, this integer was typically only observable at the boundary of the material system and not within the bulk.
\\\\
Further investigations revealed that these quantized plateaus were manifestations of new phases of matter that defied classification based on local order parameters, thereby challenging the conventional understanding of phases matter described by Landau's paradigm. Later on many more such phases of matter where found. They were all characterized by topological invariants. Some famous examples included the fractional quantum Hall effect (see figure \ref{fig:TopologicalOrderFigures}b), the majorana edge mode model (see figure \ref{fig:TopologicalOrderFigures}c), and the Haldane phase (see figure \ref{fig:TopologicalOrderFigures}d). While the majorana edge mode model remains mainly theoretical, the fractional quantum Hall effect and the Haldane phase\cite{sompet2022realizing} have been experimentally observed.
\\\\
Theoretical descriptions of these new phases of matter significantly differ from classical Landau phases, yet they share striking similarities among themselves. All of these quantum phases exhibit properties such as the invariance of their topological invariants under local perturbations and a discontinuous behavior near the boundary of the material system.
\\\\
As the name suggests, the quantum Hall effect is inherently quantum mechanical in nature and is related to a topological property of the gapped many-body ground state. As such, these phenomena are predominantly considered zero-temperature phases of matter, although certain caveats exist, such as the role of disorder, which allows the observation of quantum Hall plateaus even in situations without a gap. For non-interacting electrons in periodic potentials, the topological property is characterized by the first Chern number of the Bloch bundle. However, for ground states of more general interacting Hamiltonians, distilling the properties that distinguish the phases is far more challenging and remains an active area of research (see eg. \cite{sopenko2023chiral} where produce examples of nontrivial bosonic invertible topological order).
\\\\
As these Hall plateaus were differentiated by some topological property of the many-body ground state, they were called topological phases of matter. In the following years, many such phases of matter were discovered, each belonging to distinct topological properties of their respective many-body ground states, sometimes involving multiple ground states with certain degeneracy. Some notable examples include invertible topological order, which is believed to be related to the integer quantum Hall effect, and anyonic topological order, associated with the fractional quantum Hall effect. Symmetry-protected topological order is linked to the majorana edge modes and the Haldane phase, while symmetry-enriched topological order and many other intriguing topological phases have also been explored.
...
\begin{lemma}[Uniqueness of the GNS triple]\label{lem:UniquenessOfGNSTriple}
	Let $(\HH_1,\pi_1,\ket{v_1})$ and $(\HH_2,\pi_2,\ket{v_2})$ be two GNS triples for $\omega\in\SS(\AA)$ then there exists a unique unitary $U\in\UU(\HH_1,\HH_2)$ such that
	\begin{align}
...
\end{equation}
is a bijection\footnote{With the right topologies it is even a Homeomorphism.}. Here $\PP\HH_\omega$ is the states in $\HH_\omega$ up to a phase (also known as the rays). In other words, the vectors in a GNS Hilbert space label all states that are equal to each other at infinity.
