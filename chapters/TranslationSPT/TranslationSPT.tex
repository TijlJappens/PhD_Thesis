...
\end{equation}
Following \cite{Ogata2d} we will fix a specific family of monotonically decreasing positive functions by saying that for any $0<\phi<1$ we define
...
\end{equation}
with initial condition $\gamma^{\Phi}_{s;s}(A)=A$ (the existence of this automorphism is proven in \cite{nachtergaele2019quasi}). This satisfies the condition that if $\sup_t\norm{\sum_{I\in\mathfrak{B}_{\ZZ^2}}\Phi(I,t)}<\infty$ then
\begin{align}
...
\begin{definition}\label{def:EquivalenceOfG_InvariantSRE_States}
	Take $(\omega_1,U_1)$ and $(\omega_2,U_2)$ elements of
	\begin{equation}\label{eq: DefinitionOfS_0}
...
\begin{align}
S_1&=\left\{(\omega,U)\in\PP(\AA)\times\hom(G,U(\CC^d))\left|\begin{matrix}
	\omega\text{ is SRE},\:\omega\circ i_{U(g)}=\omega\:\forall g\in G,\\\omega\circ\tau=\omega
\end{matrix}\right.\right\}\\
S_2&=\left\{(\omega,U)\in\PP(\AA)\times\hom(G,U(\CC^d))\left|\begin{matrix}
	\omega\text{ is SRE},\:\omega\circ i_{U(g)}=\omega\:\forall g\in G,\\\omega\circ\nu=\omega
\end{matrix}\right.\right\}\\
\label{eq: DefinitionOfS_i}
S_3&=\left\{(\omega,U)\in\PP(\AA)\times\hom(G,U(\CC^d))\left|\begin{matrix}
	\omega\text{ is SRE},\:\omega\circ i_{U(g)}=\omega\:\forall g\in G,\\\omega\circ\tau=\omega,\:\omega\circ\nu=\omega
\end{matrix}\right.\right\}
\end{align}
...
\begin{lemma}\label{lem:IndicesConstantOnStableEquivalenceClasses}
	...
	\begin{enumerate}
		...
		\end{equation}
		where $\textrm{Index}_{\text{no trans}}$ is the SPT index introduced in \cite{Ogata2d} (see subsection \ref{sec:SomeRemarks} for why this is well defined in this context).
...
\end{proof}
\subsection{Comparison with \cite{Ogata2d}}\label{sec:SomeRemarks}
The first thing we would like to remark is that in \cite{Ogata2d}, a different equivalence class on states is used. First of all the set of states considered is a priori different. Namely, for any state $\psi\in \PP(\AA)$, we define two different conditions
\begin{enumerate}
...
There where two things that are in theorems \ref{thrm:ExistenceFirstIndex} and \ref{thrm:ExistenceSecondIndex} but where not discussed in \cite{Ogata2d}. This is the part that the index is a group homomorphism under stacking and the part that the index is independent on the choice of basis for the on site Hilbert space. However, on inspection of the arguments used in subsections \ref{sec:StackingOneTranslation} and \ref{sec:the-h2-valued-index-is-invariant-of-choice-of-basis-of-on-site-hilbert-space} one can conclude that these also hold for the $H^3$-valued index.\\\\
...
\end{proof}
Following Yoshiko Ogata \cite{Ogata2d} we now define objects using the following lemma:
...
\end{lemma}
\begin{proof}
	Take lemma 2.4 from \cite{Ogata2d}. There it is stated that there exists a 3-cochain $C'$ such that
...
\end{proof}
By the result of \cite{ogata2021classification} this implies that $\phi$ has a well defined 1d SPT index (see appendix \ref{sec:OneDimensionalIndices}). For any $j\in\ZZ$ and $n\in\NN$ take
\begin{align}
...
\begin{proof}
	Completely analogous to the proof of lemma \ref{lem:SRE_Implies_QDisentanglable_OneTranslation} except that the support area of the $\eta_g$ automorphism has become smaller. To deal with this, replace the $\textrm{SQAut}(\AA)$ in theorem 3.1 of \cite{Ogata2d} by $\textrm{SQAut}_2(\AA)$ (see definition \ref{def:SQAut2}) and observe that the resulting $\eta_g$ has the right support.
\end{proof}
